...
EOF
if gcloud endpoints services describe ${FQDN} 2> /dev/null; then
  echo "Skipping ${FQDN} deployment"
else
...
if kubectl get managedcertificate $K8S_CERTIFICATE 2> /dev/null; then
  echo "Managed cert $K8S_CERTIFICATE exists "
else
